...
#define kJSAnimatedImagesViewImageViewsBorderOffset 0
...
@interface JSAnimatedImagesView()
{
    ...
    NSInteger currentIndex;// mxy add
}
...
- (void)bringNextImage
{
    ...
    do
    {
       // nextImageToShowIndex = [[self class] randomIntBetweenNumber:0 andNumber:totalImages-1];
        ...
        nextImageToShowIndex =currentIndex==totalImages?currentIndex=0:currentIndex++;
    }
    while (nextImageToShowIndex == currentlyDisplayingImageIndex);
    
    currentlyDisplayingImageIndex = nextImageToShowIndex;

   // NSURL *url=[self.delegate animatedImagesViewWithUrl:self imageAtIndex:nextImageToShowIndex];
   // NSLog(@"url==%@",url);
   // imageViewToShow.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:url]];//[self.delegate animatedImagesView:self imageAtIndex:nextImageToShowIndex];
    
    [imageViewToShow setImageWithURL:[self.delegate animatedImagesViewWithUrl:self imageAtIndex:nextImageToShowIndex]];
