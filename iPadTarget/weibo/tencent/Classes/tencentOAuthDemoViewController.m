...
-(void)getListAlbumResponse:(APIResponse*) response {
	...
	if (response.retCode == URLREQUEST_SUCCEED)
	{
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"操作成功" message:[str decodeUnicode]
...
-(void)addTopicResponse:(APIResponse*) response {
	if (response.retCode == URLREQUEST_SUCCEED)
	{
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"操作失败" message:response.errorMsg													   delegate:self cancelButtonTitle:@"我知道啦" otherButtonTitles: nil];
		[alert show];
	}
