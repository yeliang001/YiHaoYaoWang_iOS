					  ...
					  animated:(BOOL)animated
{
	...
	UIView *keyView = aParentView ? aParentView : self.keyView;
...
- (void)presentPopoverFromRectV2:(CGRect)rect 
						inView:(UIView *)theView 
	  permittedArrowDirections:(UIPopoverArrowDirection)arrowDirections 
					  animated:(BOOL)animated {
...
//    加入支持presentmodalview方法产生的modalview上显示
    for (UIView* v in [w subviews]) {
        if ([[[v class] description] isEqualToString:@"UIDropShadowView"]) {
            return v;
        }
    }
//    －－－－－－－－－－－－－－－－－－－－－－－－
