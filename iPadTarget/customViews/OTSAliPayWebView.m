...
#define STR_ALIPAY_PARTNER              @"2088501903418573"//@"2088701462230533"
...
-(OTSLoadingView *)phoneLoadingView
{
    if (_phoneLoadingView == nil)
    {
        _phoneLoadingView = [[OTSLoadingView alloc] init];
    }
    
    return _phoneLoadingView;
}
...
-(void)showLoadingView
{
    ...
    {
        [self.phoneLoadingView showInView:self];
...
//    
//    NSLog(@"replace count:%d page html:\n\n%@\n\n", result, htmlStr);
//    
...
{
    NSLog(@"web view:handleAliPayLoginResult");
...
- (BOOL)webView:(UIWebView *)aWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if (range.location != NSNotFound)
    {
        ...
        if (returnInfoArrs.count > 1)
        {
            [returnInfoArrs replaceObjectAtIndex:1 withObject:[self _nickName]];
        }
...
-(NSString *)_nickName
{
    if (self.loginWithAliButNotTaobao)
    {
        return [OTSUnionLoginHelper sharedInstance].userNameAli;
    }
    
    return [OTSUnionLoginHelper sharedInstance].userNameTaobao;
}
