...
- (void)viewDidLoad {
    ...
    [yihaodianTypeBtn setTitle:@"1号药店" forState:UIControlStateNormal];
    ...
    while (running) {		
		switch (self.currentState) {
			...
			{    
				...
				@try
                {
                    ...
                    NSDictionary *dic = @{   @"userid" : [GlobalValue getGlobalValueInstance].userInfo.ecUserId,
                                           @"pageindex": [NSString stringWithFormat:@"%d", m_ShowPageCount],
                                            @"pagesize": @"10",
                    ...
                    if (m_LoadingMore)
                    {
                        [self.favoriteList addObjectsFromArray:favoriteResult.favoriteList];
                    }
                    else
                    {
			...
			case FAVORITE_DEL_STATE://删除收藏
			{
                ...
                {
                    currentState=MYFAVORITE_STATE;
                    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:[NSString stringWithFormat:@"%@%@",[NSString stringWithFormat:@"%d",self.delProductId],[GlobalValue getGlobalValueInstance].userInfo.ecUserId]];
                }
                [self setUpThread:YES];
				[pool drain];
			...
			{
			 NSAutoreleasePool * pool = [[NSAutoreleasePool alloc] init];
			 FavoriteProductInfo *favorite = [self.favoriteList objectAtIndex:currentActionIndex];

                    // 购物添加到本地
                
                YWLocalCatService *localCatService = [[YWLocalCatService alloc] init];
                LocalCarInfo *localCart = [[LocalCarInfo alloc] initWithProductId:favorite.pid
                                                                    shoppingCount:@"1"
                                                                         imageUrl:favorite.productImgUrl
...
//    if (favoriteInfo.isOnSale && favoriteInfo.currentStock>0)
//    {
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row==[self.favoriteList count]-1 && [self.favoriteList count]<favoriteCellNum)
    {
        if (!m_LoadingMore)
        {
...
-(void)accessoryButtonTap:(UIControl *)button withEvent:(UIEvent *)event{
    ...
    else
    {
		...
		FavoriteProductInfo *favorite = [self.favoriteList objectAtIndex:indexPath.row];

        if (!favorite.isOnSale)
        {
            [self showAlertView:nil alertMsg:@"很抱歉,该商品已经下架了" alertTag:ALERTVIEW_TAG_OTHERS];
        }
        else if (!favorite.currentStock > 0)
        {
